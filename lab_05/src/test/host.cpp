...
int main(int argc, char** argv) {
    ...
    var004_no_pragmas(gold.data(), source_a.data(), source_b.data(), DATA_SIZE);
   ...
   // ------------------------------------------------------------------------------------
    OCL_CHECK(err, cl::Kernel kernel_var004_no_pragmas(program, "var004_no_pragmas", &err));

   // Устанавливаем необходимые нам значения 
    OCL_CHECK(err, err = kernel_var004_no_pragmas.setArg(0, buffer_result));
    OCL_CHECK(err, err = kernel_var004_no_pragmas.setArg(1, buffer_a));
    OCL_CHECK(err, err = kernel_var004_no_pragmas.setArg(2, buffer_b));
    OCL_CHECK(err, err = kernel_var004_no_pragmas.setArg(3, DATA_SIZE));
   ...
   OCL_CHECK(err, err = q.enqueueTask(kernel_var004_no_pragmas, nullptr, &event));
    q.finish();
    ...
    printf("| %-22s  | %23lu |\n", "var004_no_pragmas", no_pragmas_time);
    printf("|-------------------------+-------------------------|\n");
   ...
   // ------------------------------------------------------------------------------------
    ...
    OCL_CHECK(err, cl::Kernel kernel_var004_urolled(program, "var004_urolled", &err));

   // Устанавливаем необходимые нам значения 
    OCL_CHECK(err, err = kernel_var004_urolled.setArg(0, buffer_result));
    OCL_CHECK(err, err = kernel_var004_urolled.setArg(1, buffer_a));
    OCL_CHECK(err, err = kernel_var004_urolled.setArg(2, buffer_b));
    OCL_CHECK(err, err = kernel_var004_urolled.setArg(3, DATA_SIZE));

    // Второй раз копировать память не требуется: DDR память используется всеми ускорителями
    // Запускаем задачу на исполнение и ждем готовности по прерыванию
   OCL_CHECK(err, err = q.enqueueTask(kernel_var004_urolled, nullptr, &event));
    q.finish();
   ...
   // ------------------------------------------------------------------------------------
    ...
    OCL_CHECK(err, cl::Kernel kernel_var004_pipelined(program, "var004_pipelined", &err));

   // Устанавливаем необходимые нам значения 
    OCL_CHECK(err, err = kernel_var004_pipelined.setArg(0, buffer_result));
    OCL_CHECK(err, err = kernel_var004_pipelined.setArg(1, buffer_a));
    OCL_CHECK(err, err = kernel_var004_pipelined.setArg(2, buffer_b));
    OCL_CHECK(err, err = kernel_var004_pipelined.setArg(3, DATA_SIZE));

    // Второй раз копировать память не требуется: DDR память используется всеми ускорителями
    // Запускаем задачу на исполнение и ждем готовности по прерыванию
    OCL_CHECK(err, err = q.enqueueTask(kernel_var004_pipelined, nullptr, &event));
    q.finish();
    ...
    printf("| %-22s  | %23lu |\n", "var004_pipelined", pipelined_time);
    printf("|-------------------------+-------------------------|\n");
   ...
   // -------------------------------------------------------------------------------------------------
    ...
    OCL_CHECK(err, cl::Kernel kernel_var004_pipe_unroll(program, "var004_pipe_unroll", &err));

   // Устанавливаем необходимые нам значения 
    OCL_CHECK(err, err = kernel_var004_pipe_unroll.setArg(0, buffer_result));
    OCL_CHECK(err, err = kernel_var004_pipe_unroll.setArg(1, buffer_a));
    OCL_CHECK(err, err = kernel_var004_pipe_unroll.setArg(2, buffer_b));
    OCL_CHECK(err, err = kernel_var004_pipe_unroll.setArg(3, DATA_SIZE));

    // Второй раз копировать память не требуется: DDR память используется всеми ускорителями
    // Запускаем задачу на исполнение и ждем готовности по прерыванию
    OCL_CHECK(err, err = q.enqueueTask(kernel_var004_pipe_unroll, nullptr, &event));
    q.finish();
    ...
    printf("| %-22s  | %23lu |\n", "var004_pipe_unroll", pipe_unroll_time);
    printf("|-------------------------+-------------------------|\n");
    printf(
